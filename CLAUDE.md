...
**X（SNS）への自動投稿はデフォルトで行わない**。PR には常に `skip-post` を付ける（ラベル＋可能ならタイトル）。
投稿したいときはユーザーがそう言うので、こちらから毎回聞く必要はない。
...
- PR タイトルは一般人に伝わる書き方（コード用語を避け、具体数値と業務影響。X 投稿するときそのまま本文になる）
- **stg 向け PR は、cron のクローリング処理に影響しない変更なら常に skip-ci**（CI は Mock クローリングのテストなので、
  そこに関係しない変更を待つ意味がない）。確実に効かせるため PR タイトルを `skip-ci:` 始まりにする
- **PR には常に `skip-post` ラベルを付ける**（X 自動投稿はデフォルト無効。投稿するのはユーザーが明示したときだけ）
- **本番へ上げる前に CI を手動実行して success を確認してからマージする**（`gh workflow run ci.yml --ref stg`）。
  昇格 PR（head=stg）は CI が自動 skip され、デプロイ側のゲートも skipped を通すので、手動実行しないと
  一度もテストされないまま本番へ出てしまう
