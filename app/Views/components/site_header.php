...
<header class="site_header_outer" id="site_header">
    <div class="site_header">
        ...
        <div class="header-actions"<?php if ($hideSearchButton ?? false) echo ' style="margin-right: 0;"' ?>>
            ...
            <?php // テーマ切替はホームのみ（3状態: ライト→ダーク→自動。実装は public/js/theme.js、アイコン出し分けは site_header.css の [data-theme-pref]） ?>
            <?php if (in_array(path(), ['', '/'], true)) : ?>
                <button class="header-button unset theme-toggle-btn" type="button" aria-label="<?php echo t('ダークモード切替') ?>">
                    ...
                    <svg class="theme-icon-auto" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 0 0 18z" fill="currentColor" stroke="none"/></svg>
                </button>
            <?php endif ?>
        </div>
