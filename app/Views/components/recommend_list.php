        ...
        <ul class="recommend-list">
            <?php foreach ($recommend->getList(false) as $roc) : ?>
                <?php $roomHref = \App\Services\Seo\OpenChatUrlNormalizer::roomUrl(
                    (int)$roc['id'],
                    $roc['table_name'] === AppConfig::RANKING_HOUR_TABLE_NAME || $roc['table_name'] === AppConfig::RANKING_DAY_TABLE_NAME ? ['limit' => 'hour'] : []
                ) ?>
                <li class="<?php if ($roc['id'] === $id) echo 'selected' ?>">
                    <a class="rectangle" <?php if ($roc['id'] !== $id) echo 'href="' . $roomHref . '"' ?>>
