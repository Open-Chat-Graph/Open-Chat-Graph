...
/**
 * GD による多言語テキスト・画像描画の共通エンジン（抽象基底クラス）。
 *
 * 元は OcCardImageGenerator（OGP カード 1200x630）内の実装だったものを、TikTok 用縦型動画スライド
 * （app/Services/TikTokVideo/TikTokVideoSlideGenerator 1080x1920）と共有するために抽出した。
 * 「データ → GD 画像」を作るクラスはこれを継承し、キャンバスサイズ・レイアウトだけを各自で持つ。
 *
 * 提供するもの:
 *  - 多言語フォントフォールバック描画: 1文字ずつ「その字を持つ最初のフォント」を選ぶ
 *    （日本語/中国語/ラテン/数字=Noto CJK JP、タイ語=Noto Sans Thai、絵文字=NotoColorEmoji の
 *    カラーPNG、その他記号=NotoSansSymbols2。どれも無い字はスキップ＝豆腐を出さない）
 *  - 折り返し付きタイトル描画（自動縮小・末尾…省略・英単語を途中で割らない）
 *  - 円形アイコン描画（外部URL取得・失敗時プレースホルダ円）
 *
 * フォントは storage/font 同梱（本番/ローカル/CI で同一描画）。
 */
abstract class GdTextRenderer
{
    ...
    /**
     * この環境で GD + FreeType + フォントによるテキスト描画が可能か（例外に頼らず事前判定する）。
     * FreeType 関数まで確認するのは GD が FreeType 無しビルドだと imagettftext が警告→
     * （このアプリのハンドラで）例外になるため。
     */
    protected function canRenderText(): bool
    {
    ...
    /** drawLine で描いたときの総送り幅（描画せず計測のみ・右寄せ/中央寄せ配置の計算用）。 */
     ...
     *
     * $align: 'left'（既定・元 OGP カードの挙動）または 'center'（各行をキャンバス内で中央寄せ。
     * 縦型動画スライド用に追加）。center 時の基準は $x〜$x+$maxWidth の中心。
     */
    protected function drawTitle(\GdImage $im, string $name, int $x, int $topY, int $maxWidth, int $maxSize, int $maxLines, int $color, string $align = 'left'): void
    {
        ...
        foreach ($lines as $lineIdx => $line) {
            ...
            $cx = $x;
            if ($align === 'center') {
                $lineW = (int)array_sum(array_column($line, 'w'));
                $cx = $x + max(0, intdiv($maxWidth - $lineW, 2));
            }
            foreach ($line as $op) {
                if ($op['emoji']) {
