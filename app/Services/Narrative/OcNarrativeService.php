...
class OcNarrativeService
{
    ...
    private function describeTrend(?int $diff7, ?int $diff30, ?int $diff90, ?float $pct7, ?float $pct30, ?float $pct90): ?string
    {
        ...
        if ($p30 > 0 && $p90 > 0) {
            ...
            $rate30 = $p30 / 30;
            $rate90 = $p90 / 90;
            $accelerating = $rate30 > $rate90 * 1.3;
            $decelerating = $rate30 < $rate90 * 0.7;

            // 直近 1 ヶ月の成長が依然として強い (% か実数のどちらかが大) 場合は、
            // ペースが鈍化していても「落ち着いた」とは言わず前向きに表現する。
            // (+261 人/月 のような部屋を「伸びが落ち着いた」と過小評価しないため)
            $recentStrong = $p30 >= 3.0 || ($diff30 !== null && $diff30 >= 100);
            if ($recentStrong) {
                // 率ではなく実数規模で評価 (末尾の「緩やかな増加」(率) と矛盾しない表現)
                return $accelerating
                    ? '長期にわたり成長を続けており、直近はさらにペースを上げている。'
                    : '長期にわたり着実に成長を続けており、直近 1 ヶ月も大きく人数を伸ばしている。';
            }
