...
- アラート通知: 毎時クロール後のα処理(`alpha_hourly`)で検出 — ①アラートキーワードに一致する新しい部屋（LINE公式検索APIで未登録部屋も最速。「新しい」＝ウォッチ作成以降にDB収録/初観測された部屋のみで、古い部屋が検索順位の揺れで上位に入っても通知しない。同部屋×同ウォッチは一度だけ）②アラート対象部屋の±％ ③マイリスト全体の±％ ④**ウォッチ部屋の機微シグナル3種**（`AlphaSignalDetectionService`。room_change=部屋名/説明文/カテゴリの変更（`alpha_room_snapshot_ja` に前回値を退避して比較・初回はseedのみ通知なし・ウォッチ解除部屋の行は毎時掃除）／rank_jump=公式ランキング順位の急変（SQLite ranking_position 日次。enter=2日以上不在→掲載再開/初掲載、jump=前回比30位以上かつ30%以上の上昇。評価カテゴリは0(全体)優先→部屋自身のカテゴリ。日1回 dedup）／pace=増加ペース異常（InsightsService の pace_anomaly と同式: 直近7日ペースが90日平均の2倍以上かつ7日で+15人以上。日1回 dedup））。通知タブは新着部屋＋増減＋機微シグナルを1本の時系列タイムラインに混在表示、未読バッジ・既読化。アラート設定(`/watch`)はキーワードとマイリスト全体の2つ。部屋ごとのアラートは詳細画面で設定/解除（解除は確認ダイアログ）。ユーザー識別はマイリストと同じCookie。
