...
- Labs (`/labs`): 本家 `/oc/{id}` のアクセス数ランキング(GA4 PV)・検索流入ランキング(GSC クリック/表示/平均順位)。**部屋と「ページ全体」(トップ/おすすめ等)を1つの並びに統合し指標降順**（ページは汎用アイコンで本家ページへ）。検索流入時は上位検索クエリ一覧も。日次バッチ(`alpha_ga_sync`)で部屋別集計。「分析」タブ（独立）から到達。
...
3. **Labs 検索KWを別タブ化＋無限スクロール（#34）** — 流入クエリ一覧を「アクセス/検索」とは別タブに。件数(limit)セレクタ廃止し**無限スクロールで全件**。検索条件ヘッダを通常検索と同様に上部固定。
...
5. **アラート設定画面の拡充（#36）** — `/watch` で①キーワードアラートを追加可能に②設定済みルーム一覧を出しそこから解除③最終更新/設定のヘッダーバーを固定。
6. **マイリスト変動アラート3パターン（#37）** — フォルダごと/全体/ルートのみ。詳細と**同じ%/人数プルダウンの共通コンポーネントを使い回す**。部屋単体設定とマイリストで二重通知しない（どちらか一方）。**バックエンド/スキーマ要**（現 `alpha_mylist_threshold` は%のみ・全体スコープ→フォルダ/ルート/人数対応へ拡張、`AlphaAlertService` で重複排除）。
7. **詳細/Labs 指標の期間プルダウン（#38）** — 詳細のアクセス・検索指標を既定「過去1ヶ月」にし、1ヶ月 or カレンダー（開始〜終了、開始は最古データ）で範囲指定。Labsも同様にカレンダー範囲。期間に含まれるデータは全部集計。**バックエンド**: room-metrics/ranking に開始-終了日(date range)パラメータを追加（`*_daily` を範囲SUM）。
