...
- `batch/exec/alpha_hourly.php`: 毎時クロール後にアラート（キーワード/部屋±/マイリスト±＋機微シグナル3種=room_change/rank_jump/pace）を算出・保存（ja限定、`cron_crawling.php`から）。中身は `AlphaAlertService::run()`（機微シグナルは `AlphaSignalDetectionService` に委譲し notifiedUserIds に合流）。保存後、新規通知が入ったユーザーの購読へ Web Push tickle を送信（`AlphaPushService::notifyUsers`）。
