...
- 検索: キーワード＋カテゴリ絞り込み、ソート軸（人数 / 1時間・24時間・1週間増減 / 作成日）＋昇降トグル、検索条件の保存（localStorage）、無限スクロール。読み込み表示は全状態で同一の細いプログレスバーに統一（初回＝上部バー＋キャプション／再検索＝前回結果を薄くdim＋同じ上部バー／追加読み込み＝リスト末尾に同じバー。スピナーは使わない）。初期/空状態のランディングに「最近の検索」（自動履歴・チップ・タップで再検索・個別/一括削除）と「保存した検索条件」（タップで再検索・削除）を表示。カードは本家風のコンパクト行で、ソート軸に対応する増減を表示。**カード右端に7日スパークライン＋増減チップ**（`/alpha-api/sparkline?ids=...` で表示中アイテム分を後から水和。`services/sparklineCache.ts` のメモリキャッシュ＋`hooks/useSparklines.ts`。マイリストも同じ）。
- 詳細: 丸アイコンヘッダ＋画像モーダル（URL駆動）、人数＋3期間の増減、本家と同じoc-appグラフを埋め込み（人数＋順位線。操作列の高さを OcGraph.tsx の `controlsHeight()` が DTO から事前計算して予約し描画完了時のレイアウトシフト無し）、ランキング掲載履歴（遅延オーバーレイ・件数表示／0件はグレーアウト）、高次の考察ブロック、アクション（LINEで開く / マイリスト）、増減アラート（枠上部の開閉トグル。未有効時は「解除」を出さず「ONにする」のみ／有効時は人数・％しきい値の編集と「解除」）。
...
- 高次の考察: 一目で分からない高次シグナルを意味のある3軸に集約（各最大1枚・優先度この順・データ不足は黙ってスキップ＝1〜3枚）。①momentum（勢い）＝直近7日の純増を主役に、平常ペース比（直近7日/日 ÷ 過去90日/日が2倍以上なら「普段の約N倍」）と成長ランキング全体順位（週>日>時で50位以内なら添える）を統合。発火は sample_n>=7 かつ diff7>=max(15, 現在人数×0.5%)。②rank_position（公式ランキングでの位置）＝現在順位を主役に、30日推移（3位以上動いていれば「X→Y位に上昇/下降」）と自己最高位を統合。発火は sample_n>=5 かつ現在<=300位。③category_position（カテゴリ内での位置）＝カテゴリ内人数順位を主役に、シェア（1%以上で「カテゴリ全体の約N%」）を統合。発火は total>=20 かつ上位25%以内。下位・ノイズは黙る。フロントは未知typeを「考察」fallbackで描画（旧通知キャッシュ保険）。
...
- スマートフォルダ＋フォルダ単位アラート（バックエンド実装済み・フロント未配線）: フォルダにルール（キーワード必須＋カテゴリ任意。`alpha_mylist_folder_ja.rule_*` 列）を設定すると、(a)設定時に現在の一致部屋（open_chat への LIKE。検索APIと同じ空白区切りAND・name/description 部分一致）・人数上位50件を即時追加 (b)毎時 `alpha_hourly` が「rule_created_at 以降にDB収録（open_chat.created_at）された新着の一致部屋」を自動追加。追加はすべて `alpha_mylist_item_ja` に source='auto'。再追加防止は `alpha_folder_seen_ja`（恒久。ユーザーが消した部屋は二度と戻さない）。既にマイリストのどこかにある部屋は動かさない（INSERT IGNORE）。フォルダ単位アラートは `alpha_folder_threshold_ja`（意味は mylist threshold と同じ %/人数 NULL許容・AND判定）で、**子孫フォルダ込み**（サーバ側でフォルダ木を再帰解決）の配下アイテムを毎時判定。通知 type は `folder_add`（dedup `fa:{folderId}:{ocId}`=恒久一意）と `folder`（payload は mylist movement と同形＋folderId/folderName。dedup `fm:{folderId}:{ocId}:{direction}:{hourBucket}`）。**通知の優先順位は room > folder > mylist**（同毎時に同(user,部屋)で上位typeが通知済みなら下位はスキップ。folder 同士＝複数フォルダは両方通知）。実装: `AlphaSmartFolderService` / `AlphaSmartFolderRepository`。ルール解除（rule:null）で auto アイテムは残る。フォルダ削除（mylist PUT 全置換）で rule・threshold・seen・フォルダ内 auto アイテムを掃除。**発見導線（2026-06-08 UX監査 #12）**: フォルダ作成ダイアログに「作成後、キーワードを設定すると一致する新着部屋が自動で入ります」の案内を表示し、新規作成直後はそのままフォルダ設定ダイアログを自動で開く（作成→設定の1動線。`MyListPage.handleFolderCreated`。連続ダイアログは useBackDismiss の履歴巻き戻しと競合するため popstate 静定後に flushSync で「閉じた状態の mount→open」の2段階で開く）。
- アラート通知: 毎時クロール後のα処理(`alpha_hourly`)で検出 — ①アラートキーワードに一致する新しい部屋（LINE公式検索API。「新しい」＝ウォッチ作成以降にDB収録/初観測された部屋のみで、古い部屋が検索順位の揺れで上位に入っても通知しない。同部屋×同ウォッチは一度だけ。検索に出たオプチャグラフ未登録の部屋は通知せず、自動登録キュー`alpha_search_seen_room_ja`に積み、毎時クロール末尾(`SyncOpenChat::hourlyTask`→`AlphaSeenRoomRegistrationService`)で本体へ自動登録する＝登録後は次回以降この登録済み経路で通知される）②アラート対象部屋の±％ ③マイリスト全体の±％ ④**ウォッチ部屋の機微シグナル3種**（`AlphaSignalDetectionService`。room_change=部屋名/説明文/カテゴリの変更（`alpha_room_snapshot_ja` に前回値を退避して比較・初回はseedのみ通知なし・ウォッチ解除部屋の行は毎時掃除）／rank_jump=公式ランキング順位の急変（SQLite ranking_position 日次。enter=2日以上不在→掲載再開/初掲載、jump=前回比30位以上かつ30%以上の上昇。評価カテゴリは0(全体)優先→部屋自身のカテゴリ。日1回 dedup）／pace=増加ペース異常（InsightsService の pace_anomaly と同式: 直近7日ペースが90日平均の2倍以上かつ7日で+15人以上。日1回 dedup））。通知タブは新着部屋＋増減＋機微シグナルを1本の時系列タイムラインに混在表示、未読バッジ・既読化。アラート設定(`/watch`)はキーワードとマイリスト全体の2つ。部屋ごとのアラートは詳細画面で設定/解除（解除は確認ダイアログ）。ユーザー識別はマイリストと同じCookie。
- Labs (`/labs`): 日次バッチ(`alpha_ga_sync`)で収集した GA4/GSC データを閲覧するα独自の分析画面。「分析」タブ（独立）から到達。3タブ構成：①**ランキング（rooms）** — 部屋単位でアクセス数(PV)・検索流入(SEO合計)・入室数(参加リンク押下)の降順ランキング。指標セレクタ（アクセス数/検索流入/入室数）で並び軸を切り替え。カテゴリ絞り込み・部屋名キーワード絞り込み付き。各カードに全指標＋流入キーワード（top8）を表示。②**その他ページ（pages）** — トップ/おすすめ等の非部屋ページのアクセス数・検索流入・入室数（流入PV比の近似値）ランキング。③**検索KW（keywords）** — サイト全体の上位検索クエリ(GSC)一覧。期間は既定30日＋カレンダー任意範囲（全期間含む）。無限スクロール。外部検索エンジンのルートURL referrer はホスト検査で除外しトップページ入室への誤計上を防止。
- PWA: manifest＋Service Worker（scope `/alpha`）。
- αトーン「Quiet Signal」: ダーク基調、アクセント=indigo–violet、数値=Sora(等幅)、本文=Noto Sans JP、shadcn/ui。用語は「アラート」に統一。**データ色トークン**: 増減は `text-up`/`text-down`（`lib/colors.ts` の `diffColorClass`/`diffBgClass` 経由。green/red直書き禁止）、アクセントは primary 1色（考察アイコン等のレインボー禁止）。**面の2段階**: 主役= `.surface-highlight`（primaryティント＋ring。ダークは濃度別定義）／副次= `.surface-tonal`（border無しのトーン面）。詳細グラフは `data-oc-palette="alpha"` 属性で oc-app 側が indigo-violet 線に切替（本家は緑のまま）。
...
- **タブ切替は「破棄＋SWRキャッシュ」モデル**（画面表示状態カーネル `src/lib/viewNavigation.ts`＋`hooks/useViewNavigation.ts`。全タブ押下がここを経由）: タブに入る（enter）も同タブ再押下（reclick）も、ビュー配下の keep-alive パネルの reset nonce（LayoutContext の `resetNonces`）を bump → `key` で強制再マウント＋scrollTop し、ルートパスへ遷移（enter は push＝ブラウザバックで元タブのURLへ戻れる／reclick は replace）。前の画面状態（検索クエリ・分析サブ画面・スクロール）は持ち越さず、データは SWR グローバルキャッシュが持つので同条件の再実行は即表示。分析タブは enter/reclick で analysis/period-growth/labs の3パネルをまとめて破棄（/watch は通知・設定の両方から到達するため対象外。なお /watch は即時保存化済みで未保存編集は持たない）。**ブラウザ/履歴の戻るは破棄しない**＝オーバーレイ（詳細等）から戻れば検索結果へ状態ごと戻る。履歴で `/?q=` に戻った場合はそのクエリで表示（キャッシュ即描画）。旧 enter 記憶（検索クエリ・`analysisLastSub`・マイリスト最終フォルダの sessionStorage 復元、`recordViewState`/`resolveEnterPath`）は廃止。検索条件の適用は `hooks/useApplySearchParams.ts` に集約（条件が変われば URL 遷移のみ＝キャッシュ即表示、同一条件のみ searchNonce bump で強制再フェッチ。検索バー/最近の検索チップ/保存条件が共用）。
...
- 一覧3画面（検索/period-growth/Labs）の無限リストは共通コントローラ `hooks/useInfiniteList.ts` に集約（旧 `useInfiniteReveal`＋ページごとの useSWRInfinite 配線を吸収）。「ネットワーク1000件取得・描画30件ずつ reveal」分離方式: バッファに未表示分があれば即時 +30、使い切ったとき・hasMore があれば次の1000件をネットワーク取得。SWRオプションは標準の stale-while-revalidate（`revalidateIfStale` は既定 true。タブ切替が「破棄して再マウント」になったため、再マウント時はキャッシュ即描画＋stale なら背景再検証。データが既にあるキーの再検証は `phase:'first'` にしない＝バー/dim を出さない。`revalidateFirstPage:false` は維持＝append のたびに1ページ目の重いクエリが再発火する二重フェッチを防ぐ。その副作用で全ページキャッシュ済みキーの mount 再検証はネットワーク無しのキャッシュ解決で完了する）・読み込みフェーズ分類（`phase: 'idle'|'first'|'more'`）・IntersectionObserver（1つを保持し揮発状態は ref 経由・状態変化時は observe 張り替えで連鎖発火）もここが持つ。表示件数のリセットは listKey の「値が実際に変化したとき」だけ（`<Activity>` 復帰のエフェクト再実行ではリストを切り詰めない）。条件変更時に `setSize(1)` は呼ばない（swr/infinite はページ数をキーごとにキャッシュし新キーは size=1 から始まる。setSize は内部 mutate が dedupe を迂回し、キー変更再検証と重なって同じ重いクエリが二重フェッチになるため）。period-growth は keep-alive で隠れている間に他ページの URL クエリ（/?q=…）を取り込まないよう「ルート所有時のみ URL パラメータ採用・非所有時は最後に所有していた値を保持」のガード付き。
- リスト取得の応答待ち表示は `components/Common/ListProgress.tsx` に集約し、3状態すべて同一の `ListProgressBar`（細い primary バー・スピナー不使用）に統一: 初回＝`ListProgressRegion` の上部バー＋キャプション／再取得＝`ListRefetchBar`（前回結果を薄くdim＋同じ上部バー）／追加読み込み（無限スクロール次ページ）＝`ListProgressFooter`（リスト末尾に同じバー。次ページETAは取れないので約900msの軽いindeterminate ramp）。検索・Labs・period-growth が共用し同一描画。先頭ページ進行値は `useListProgress`: すべて `loading`（=phase==='first'）のエッジから導出し、false→true でバー表示＋ETA取得＋0→90%、true→false で100%スナップ→ホールド→非表示（最小表示時間あり）。実フェッチが無いキャッシュ即答・`<Activity>` 復帰・データが既にあるキーの背景再検証ではバーも ETA リクエストも構造的に発生しない（幽霊ローディング防止）。loading 中のエフェクト再実行（Activity 復帰）は開始時刻 ref から再アームし途中%から再開（凍結しない）。旧 `InfiniteScrollLoader`（Loader2スピナー）は廃止。
...
| POST | `/alpha-api/batch-stats` | 複数IDの一括基本情報（API上限50件/回。フロント `alphaApi.batchStats` が50件ずつ分割並列フェッチ→結合し件数無制限） |
...
| GET | `/alpha-api/alerts` | 通知一覧（新着部屋＋増減＋機微シグナル）。応答: `keywordHits`/`movements`（payload展開済み）＋`signals`（type IN room_change/rank_jump/pace。各要素 `{id,type,isRead,createdAt,payload}` で **payload はネストのまま**返す＝フロント契約）。`unreadCount` は全type込み |
...
| GET | `/alpha-api/folder-settings/{folderId}` | スマートフォルダ設定取得 `{rule:{keyword,category,enabled}\|null, threshold:{upPercent,downPercent,upMember,downMember,enabled}\|null}`。他人/不在フォルダは404 |
| PUT | `/alpha-api/folder-settings/{folderId}` | 同形 body で保存 → `{ok:true, autoAdded:n}`。rule が新規 enable または keyword/category 変更時に初回フィル（現在の一致部屋・人数上位50件を source='auto' 追加。既にフォルダ/seen/マイリストにある部屋は除く）。rule:null で解除（auto アイテムは残す）。threshold:null で解除 |
...
| GET | `/alpha-api/sparkline` | 複数部屋の直近7点メンバー数系列（リストカードのスパークライン用。`ids=`カンマ区切り最大50件・SQLite statistics から一括） |
...
- ユーザー系(13): `alpha_keyword_watch_ja` / `alpha_room_watch_ja` / `alpha_mylist_threshold_ja` / `alpha_keyword_seen_ja` / `alpha_search_seen_room_ja`（未登録部屋の本体自動登録キュー。emid(PK)/name/fail_count/first_seen_at。毎時クロール末尾で古い順に消化＝成功/既登録は削除、取得失敗はfail_count++し上限3で諦め削除） / `alpha_search_timing_ja` / `alpha_notification_ja` / `alpha_push_subscription_ja`（Web Push購読。endpointが長いためUNIQUEは`endpoint_hash`=SHA-256。送信失敗5回 or 404/410で自動削除） / `alpha_mylist_folder_ja`（フォルダ定義。PK: user_id+folder_id。スマートフォルダのルール列 rule_keyword/rule_category/rule_enabled/rule_created_at 込み。PUT全置換の upsert は rule_* を更新しない） / `alpha_mylist_item_ja`（アイテム。PK: user_id+open_chat_id。source='manual'|'auto'。KEY user_folder(user_id,folder_id)） / `alpha_folder_seen_ja`（スマートフォルダの再追加防止。PK: user_id+folder_id+open_chat_id。フォルダ削除時に掃除） / `alpha_folder_threshold_ja`（フォルダ単位アラートしきい値。PK: user_id+folder_id。意味は mylist threshold と同じ） / `alpha_room_snapshot_ja`（room_change 検知用の name/description/category スナップショット。1部屋1行・UNIQUE open_chat_id。ウォッチ解除部屋は毎時掃除）
...
- `batch/exec/alpha_hourly.php`: 毎時クロール後にアラート（キーワード/部屋±/マイリスト±＋機微シグナル3種=room_change/rank_jump/pace＋スマートフォルダ=自動追加 folder_add/フォルダ変動 folder）を算出・保存（ja限定、`cron_crawling.php`から）。中身は `AlphaAlertService::run()`（機微シグナルは `AlphaSignalDetectionService`、スマートフォルダは `AlphaSmartFolderService` に委譲し notifiedUserIds に合流。実行順は room → smart folder → mylist で優先順位 room > folder > mylist の重複排除を成立させる）。保存後、新規通知が入ったユーザーの購読へ Web Push tickle を送信（`AlphaPushService::notifyUsers`）。
- **Web Push（ペイロード無し tickle 方式）**: RFC8291 暗号化はせず、VAPID(RFC8292) の ES256 JWT を付けた空POSTを push エンドポイントへ送るだけ。通知の中身は SW が `/alpha-api/alerts` を fetch して表示。実装は `AlphaPushService`（JWT生成: openssl_sign のDER署名を JOSE raw r||s 64byte に変換。audごとに実行内キャッシュ）＋`AlphaPushRepository`。VAPID鍵は `SecretsConfig::$vapidPublicKey/$vapidPrivateKey(PEM)/$vapidSubject`（local-secrets.php。生成コマンドは SecretsConfig のコメント参照）。未設定なら `push/config` が enabled:false を返し送信はスキップ。
...
- **【完了 2026-06-08】ウォッチ部屋の機微検知バックエンド** — `AlphaSignalDetectionService`（room_change/rank_jump/pace）を `AlphaAlertService::run()` の④として組み込み（notifiedUserIds 合流＝Push tickle 自動対象、戻り値に `signals` 件数追加。batch/exec は無変更）。新テーブル `alpha_room_snapshot_ja`（schema-sync で +1 table 反映済み）＋新リポジトリ `AlphaRoomSnapshotRepository`、SQLite ranking_position 読みは `AlphaAlertRepository::getRecentRankingObservations`。`GET /alpha-api/alerts` 応答に `signals` 配列（payload ネストのまま＝フロント契約）。しきい値: rank_jump enter=日次観測2日以上空き（=24h以上不在）/ jump=30位以上かつ30%以上上昇（前回観測が2日以内のみ）/ pace=Insights と同値（ratio 2.0・diff7>=15）。ローカル実データで room_change seed→変更通知→dedup、rank_jump enter/jump、pace（スタブ）を検証済み。
- **【#31 完了】画面表示状態カーネル** — `src/lib/viewNavigation.ts`（宣言的ビュー表＋enter/reclick/back 正規化）＋`hooks/useViewNavigation.ts` に集約。全タブがここを経由。検索→period-growth「みる」も `bumpReset('period-growth')` で同契約。旧 `useNavigationHandler`/`useScrollToTopOnReclick` 廃止。**※2026-06-08 改修: enter の記憶復元（検索クエリ／フォルダ／`analysisLastSub`）は廃止し、enter も reclick と同じ「破棄」に統一**（詳細は「UI アーキテクチャ」のタブ切替の項）。
...
- グラフ描画のレイアウトシフト解消（操作列はデータ依存で高さが変わるため、OcGraph.tsx の `controlsHeight()` が DTO（統計点数・ランキング掲載有無）から実高を計算しマウント先 min-height に予約。実測 before=after）。
...
5. **【完了 2026-06-01】アラート設定画面の拡充（#36）** — `/watch`(`WatchSettingsPage`) に①キーワードのアラートを画面内の入力＋カテゴリ選択で追加（一覧から削除も）②設定済みルーム一覧（部屋名は `batch-stats` で解決・しきい値表示・個別解除）③上部 sticky ヘッダーバー（`top-0 z-subheader`、生z-[NN]不使用）を実装。保存は PUT 全置き換え（rooms も温存）。**※2026-06-08 UX監査で即時保存に変更**（保存/キャンセルバー撤去→「自動保存されます」表示。操作のたび 500ms debounce で最新フル状態を PUT・保存は直列化・失敗時は控えめなエラー＋再試行ボタン）。
...
11. **【完了 2026-06-08】UX監査3点修正** — ①【致命】マイリスト50件超で一覧が永久エラー: `batch-stats` は最大50件/回で400を返すため、`alphaApi.batchStats` を50件ずつ分割並列フェッチ→結合に（呼び出し3箇所＝MyList/FolderChart/Watch は同関数経由で全て解消）。マイリストのエラー表示に「再読み込み」ボタン（SWR mutate）追加（`components/MyList/ErrorState.tsx`）。②/watch を即時保存に統一（上記 #36 の追記参照）。③スマートフォルダの発見導線（上記スマートフォルダ節の追記参照）。
...
- **スキーマファイルに `CREATE TABLE IF NOT EXISTS` で書かない**（必ず素の `CREATE TABLE`）。`SchemaParser` のテーブル名正規表現が `IF` をテーブル名として拾い、IF NOT EXISTS のテーブル同士がキー衝突して**最後の1つ以外が silent に同期されない**（2026-06-08 に `alpha_folder_seen_ja` が落ちて発覚。既存の IF NOT EXISTS 2件も素の CREATE TABLE に修正済み。パーサ側の根本修正は未着手）。
