import { langCode } from './fetchRenderer'

const translation = {
  '1週間': {
    tw: '1週',
    th: '1W',
  },
  '1ヶ月': {
    tw: '1個月',
    th: '1M',
  },
  最新24時間: {
    tw: '最新24小時',
    th: 'Latest 24H',
  },
  全期間: {
    tw: '全部',
    th: 'All',
  },
  急上昇: {
    tw: '熱門',
    th: 'มาแรง',
  },
  ランキング: {
    tw: '排行榜',
    th: 'การจัดอันดับ',
  },
  すべて: {
    tw: '全部',
    th: 'ทั้งหมด',
  },
  カテゴリー内: {
    tw: '分類內',
    th: 'ในหมวดหมู่',
  },
  'グラフの移動・拡大': {
    tw: '移動/縮放圖表',
    th: 'เลื่อน/ขยายกราฟ',
  },
  ランキングの順位を表示: {
    tw: '顯示排名位置',
    th: 'แสดงตำแหน่งอันดับ',
  },
  順位グラフの初期表示: {
    tw: '預設圖表顯示',
    th: 'การแสดงกราฟอันดับเริ่มต้น',
  },
  順位表示なし: {
    tw: '不顯示排名',
    th: 'ไม่แสดงอันดับ',
  },
  順位バーの目盛り: {
    tw: '排名刻度',
    th: 'สเกลอันดับ',
  },
  上位を強調: {
    tw: '強調高排名',
    th: 'เน้นอันดับสูง',
  },
  等間隔: {
    tw: '等間隔',
    th: 'ระยะเท่ากัน',
  },
  縮小: {
    tw: '縮小',
    th: 'ย่อ',
  },
  拡大: {
    tw: '放大',
    th: 'ขยาย',
  },
  'ドラッグで移動・ピンチで拡大': {
    tw: '拖動移動・捏合縮放',
    th: 'ลากเพื่อเลื่อน จีบนิ้วเพื่อซูม',
  },
  'ドラッグで移動・ホイールで拡大縮小': {
    tw: '拖動移動・滾輪縮放',
    th: 'ลากเพื่อเลื่อน ล้อเมาส์เพื่อซูม',
  },
  設定: {
    tw: '設定',
    th: 'ตั้งค่า',
  },
  'メンバー数・ランキング履歴の統計グラフ': {
    tw: '成員數量/排名歷史統計圖',
    th: 'กราฟสถิติจำนวนสมาชิก/ประวัติการจัดอันดับ',
  },
  メンバー: {
    tw: '成員',
    th: 'สมาชิก',
  },
  圏外: {
    tw: '圈外',
    th: 'n/a',
  },
  メンバー数: {
    tw: '成員數量',
    th: 'จำนวนสมาชิก',
  },
  公式急上昇の順位: {
    tw: 'LINE official 熱門',
    th: 'LINE official เทรนด์',
  },
  公式ランキングの順位: {
    tw: 'LINE official 排行榜',
    th: 'LINE official การจัดอันดับ',
  },
  ローソク足: {
    tw: 'K線圖',
    th: 'แท่งเทียน',
  },
  折れ線グラフ: {
    tw: '折線圖',
    th: 'กราฟเส้น',
  },
  始値: {
    tw: '開盤價',
    th: 'ราคาเปิด',
  },
  高値: {
    tw: '最高價',
    th: 'ราคาสูงสุด',
  },
  安値: {
    tw: '最低價',
    th: 'ราคาต่ำสุด',
  },
  終値: {
    tw: '收盤價',
    th: 'ราคาปิด',
  },
  始: {
    tw: '開',
    th: 'O',
  },
  終: {
    tw: '收',
    th: 'C',
  },
  高: {
    tw: '高',
    th: 'H',
  },
  安: {
    tw: '低',
    th: 'L',
  },
  増: {
    tw: '增',
    th: '+',
  },
  減: {
    tw: '減',
    th: '-',
  },
  人: {
    tw: '人',
    th: '',
  },
  位: {
    tw: '名',
    th: 'th',
  },
  チャートの種類: {
    tw: '圖表類型',
    th: 'ประเภทกราฟ',
  },
  期間の固定: {
    tw: '固定期間',
    th: 'ช่วงเวลาคงที่',
  },
  固定しない: {
    tw: '不固定',
    th: 'ไม่คงที่',
  },
  OHLCデータがありません: {
    tw: '沒有OHLC數據',
    th: 'ไม่มีข้อมูล OHLC',
  },
  データを取得できませんでした: {
    tw: '無法取得資料',
    th: 'ไม่สามารถโหลดข้อมูลได้',
  },
  'アクセスが集中しているか、一時的な通信エラーの可能性があります。ページを再読み込みしてください。': {
    tw: '可能是存取量過大或暫時的連線錯誤，請重新整理頁面。',
    th: 'อาจมีการเข้าใช้งานจำนวนมากหรือเกิดข้อผิดพลาดในการเชื่อมต่อชั่วคราว กรุณาโหลดหน้านี้ใหม่',
  },
  再読み込み: {
    tw: '重新整理',
    th: 'โหลดใหม่',
  },
}

const sprintfTranslation = {
  '%s 位': {
    ja: (s: string) => `${s} 位`,
    tw: (s: string) => `${s}th`,
    th: (s: string) => `${s}th`,
  },
  '%s 位 / %s 件': {
    ja: (s: string, s2: string) => `${s} 位 / ${s2} 件`,
    tw: (s: string, s2: string) => `第 ${s} 位 / ${s2} 項`,
    th: (s: string, s2: string) => `อันดับที่ ${s} / ${s2} รายการ`,
  },
  '%s 時点': {
    ja: (s: string) => `${s} 時点`,
    tw: (s: string) => `截至 ${s}`,
    th: (s: string) => `ณ เวลา ${s} น`,
  },
}

export const sprintfT = (key: string, ...string: (string | number)[]): string => {
  return (
    // @ts-expect-error dynamic key access
    sprintfTranslation[key]?.[langCode ? langCode : 'ja'](...string) ?? key
  )
}

export const t = (string: string): string => {
  // @ts-expect-error dynamic key access
  return translation[string]?.[langCode] ?? string
}

export const weekdays = {
  '': ['日', '月', '火', '水', '木', '金', '土'],
  tw: ['日', '一', '二', '三', '四', '五', '六'],
  th: ['อา', 'จ', 'อ', 'พ', 'พฤ', 'ศ', 'ส'],
}
