 ...
 * このスクロール領域ごと作り直されて 0 に戻る（MyList と同方式）。よって scrollRef は持たない。
 *
 ...
 *   内部スクロールを担う）。
 */
export function ListScreen({ header, children, className }: ListScreenProps) {
  return (
    <div className="absolute inset-0 flex flex-col">
      {header != null && (
        <div className="shrink-0 z-subheader border-b bg-background">{header}</div>
      )}
      <div className={cn('flex-1 overflow-y-auto overflow-x-hidden p-3 md:p-6', className)}>
        {children}
      </div>
    </div>
  )
