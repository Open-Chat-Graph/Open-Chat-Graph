...
import { TrendingUp, TrendingDown, ListChecks, Eye } from 'lucide-react'
...
/**
 * 「増減アラート」カード。アラート中の部屋／マイリスト全体のしきい値超え増減を表示する。
 * kind で「部屋のアラート」か「マイリスト全体」かをラベル分けする。
 */
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <Badge variant="secondary" className="h-5 gap-1 px-1.5 text-[11px] font-normal">
              {movement.kind === 'mylist' ? (
                <ListChecks className="h-3 w-3" />
              ) : (
                <Eye className="h-3 w-3" />
              )}
              {movement.kind === 'mylist' ? 'マイリスト' : 'アラート'}
            </Badge>
