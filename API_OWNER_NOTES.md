...
- 上限値は `AppConfig::API_RATE_LIMIT_WINDOW_SECONDS` / `API_RATE_LIMIT_MAX_RECORDS` で変更する
- 状態は `storage/ja/api_rate_limit/`（gitignore済・ユーザー名のSHA256がファイル名）に保存。消せばリセットされる
