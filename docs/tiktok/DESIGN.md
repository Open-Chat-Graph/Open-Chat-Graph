...
立ち上げ期はリーチ面でむしろ有利。動画をあえて無音で生成しているのはこのため。
...
GitHub Actions（.github/workflows/tiktok-video.yml）
  ...
  │   │   （タイトル → 5位..1位カウントダウン → 締めCTA）
  │   └ TikTokVideoRenderer … ffmpeg（Ken Burns ズーム＋xfade）で mp4 合成
...
- 1080x1920 / 30fps / H.264 / 無音（意図的・§1）/ 約18秒 / 実測 約1.4MB
...
- **コンテンツの型追加**: カテゴリ別 TOP5（曜日ローテ）／週間ランキング（日曜）／
  「1週間で+N人」個別ルーム深掘り型
- **ナレーション**: VOICEVOX 等（無料・商用可・クレジット表記）。ただし AI 音声は
  For You 除外リスクがあるため（§5）、導入時は小規模にテストして数字で判断
