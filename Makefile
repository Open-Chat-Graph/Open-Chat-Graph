.PHONY: help init init-y init-y-n _init up down restart rebuild ssh up-mock cron cron-stop show cert ci-test phpstan css-check build-frontend build-frontend\:ranking build-frontend\:oc-app build-frontend\:all-room-stats _build-one-frontend _wait-mysql _is-mock _check-data-protection sync-setup sync-update _ensure-prod-sync-secrets shared-setup up-shared down-shared _shared-config _shared-prepare _shared-ports _up-shared
...
PROD_SYNC_CONFIG_URL ?= https://github.com/Open-Chat-Graph/Open-Chat-Graph-Config.git
