...
PROD_SYNC_ENV="${SECRETS_DIR}/prod-sync.env"
PROD_SSH_KEY="${SECRETS_DIR}/prod-ssh.key"
LOCAL_SECRETS_TMPL="${SECRETS_DIR}/local-secrets.tmpl.php"
...
for _f in "$PROD_SYNC_ENV" "$PROD_SSH_KEY"; do
    if [ ! -f "$_f" ]; then
        ...
        echo "       oc-config 側で install-prod-sync.sh を実行して機密を配置してください。" >&2
        exit 1
    fi
done
