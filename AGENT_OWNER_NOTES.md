...
3. （任意）Cloudflare の **Markdown for Agents**（エッジでのHTML→Markdown変換機能）を有効化すると
   エッジ側でも変換される。オリジン実装と競合はしない（エッジで処理されればオリジンに届かないだけ）
   - **Proプラン以上**（Free不可）。Freeプランの場合は本リポジトリのオリジン実装がその代替
   - 案AのTransform Rule（URL Rewrite）は**Freeプランでも10ルールまで利用可**
     （条件式の `http.request.headers` も使える。正規表現はBusiness+だが contains で書けるので不要）
