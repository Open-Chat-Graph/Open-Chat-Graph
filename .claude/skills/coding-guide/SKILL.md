...
手動ディスプレイ枠の現在地は `GoogleAdsenseConfig::$enableAds` のコメントに列挙してある（ヘッダーには置かない）。
おすすめページは `array_chunk(5)` で 5 件ごとに固定高さ枠（rectangle2）を挟み、最後のチャンクの後には置かない。
...
- **フェイルオープン**: グローバルフラグは「広告オフのとき true」。昇格側は `if (フラグ) return;` と書く。
  ...
  X 用の「オファーウォールだけオフ」フラグも同じ向き（true で抑止・undefined なら出る）。
