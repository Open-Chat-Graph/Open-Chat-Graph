...
> 前提: CLAUDE.md の必守ルール（本番リリースまで通しで進める・署名・skip-ci / skip-post デフォルト方針）に従う。本ガイドはその具体的な手順と書き方。
...
**デフォルト方針: 全 PR に `skip-post` を付ける（X 自動投稿はしない）。**
投稿したいときはユーザーがそう言うので、毎回確認する必要はない。ユーザーから投稿指示があった PR でだけ
`skip-post` を外す。

`skip-ci` と併用するときの付け方（タイトル先頭は 1 つしか使えない）:

- タイトルは `skip-ci:` 始まりにする（CI の判定は PR opened 時のタイトルで行われるため、こちらを優先）
- `skip-post` は**ラベルで付ける**（X 投稿の判定はマージ時に評価されるのでラベルで確実に効く）
- 両方ラベルも付けておくと、`[STG]`/`[PROD]` の自動プレフィックス付与後もゲートが確実に効く
